...
const SESSION = "demo";
// 训练样本来源:分开管理教学套路 / 纠错数据 / 工单补充
const SOURCE_LABEL = { taught: "对话教学", refine: "实测纠偏", ticket: "工单补充" };
const srcLabel = (s) => SOURCE_LABEL[s] || "对话教学";
const QUICK = [
  "有哪些课程?",
  "这个课多少钱?",
  "我零基础能学吗?",
...
async function openKbModal() {
  ...
  box.innerHTML = "";
  if (!list.length) {
    box.innerHTML = '<div class="empty">套路库还是空的,去「教学模式」教它第一条吧~</div>';
    $("#kb-modal").classList.add("show");
    return;
  }
  const groups = [
    { key: "taught", title: "🎓 教学套路" },
    { key: "refine", title: "✍️ 纠错数据" },
    { key: "ticket", title: "📮 工单补充" },
  ];
  groups.forEach((g) => {
...
async function loadReview() {
  ...
  const emptyHint = {
    taught: '<div class="empty">还没有教学套路。去「教学模式」教它几条~</div>',
    refine: '<div class="empty">还没有纠错数据。在咨询页对回复点「纠偏」并「固化为套路」即可产生。</div>',
    ticket: '<div class="empty">还没有工单补充。</div>',
  };
